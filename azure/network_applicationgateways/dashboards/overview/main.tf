...
output "dashboard_url" {
  value       = "https://app.lightstep.com/${var.lightstep_project}/dashboard/${lightstep_dashboard.azure_network_applicationgateways_overview.id}"
  description = "OpenTelemetry Collector Network Application Gateways Dashboard URL"
}

resource "lightstep_dashboard" "azure_network_applicationgateways_overview" {
  project_name          = var.lightstep_project
  dashboard_name        = "Network Application Gateways Metrics"
  dashboard_description = "[Beta] Monitor Network Application Gateways with this metrics overview dashboard."
...
EOT
  ...
  chart {
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_blockedcount_total | filter ((azuremonitor.subscription_id == $azuremonitor_subscription_id) && (azuremonitor.tenant_id == $azuremonitor_tenant_id) && (azuremonitor.resource_id == $azuremonitor_resource_id) && (location == $location)) | delta | group_by [], sum
EOT
    }
  }
...
EOT
  ...
  chart {
    name = "Avg Request Count Per Healthy Host"
    rank = "7"
    type = "timeseries"
...
EOT
  ...
  chart {
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_backendconnecttime_average | filter ((azuremonitor.subscription_id == $azuremonitor_subscription_id) && (azuremonitor.tenant_id == $azuremonitor_tenant_id) && (azuremonitor.resource_id == $azuremonitor_resource_id) && (location == $location) && (metadata_listener == $metadata_listener)) | delta | group_by [], sum
EOT
    }
  }
...
EOT
  ...
  chart {
    name = "WAF Bot Protection Matches"
    rank = "13"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_azwafbotprotection_total | filter ((azuremonitor.subscription_id == $azuremonitor_subscription_id) && (azuremonitor.tenant_id == $azuremonitor_tenant_id) && (azuremonitor.resource_id == $azuremonitor_resource_id) && (location == $location) && (metadata_action == $metadata_action)) | delta | group_by [], sum
EOT
    }
  }
