...
resource "azurerm_container_app" "otelcol_app" {
  ...
  template {
    ...
    min_replicas = 1
    container {
      command = ["/otelcontribcol", "--config", "/conf/eventhub-only.yaml"]
      cpu     = 0.5
      // TODO: configure to pin versions
      image = "docker.io/nathanslaughter/otelcontribcol-exp:latest"
      // "nathanslaughter/otelcontribcol-exp:latest"
      memory = "1Gi"
      ...
      env {
        name = "AZURE_EVENTHUB_CONNECTION_RESOURCE_LOGS"
      ...
      /*
      env {
        name = "AZURE_EVENTHUB_CONNECTION_ACTIVITY_LOGS"
        // can validate with query in root terraform dir like `terraform output eventhub_connection_string`
      ...
      */
      env {
        name = "AZURE_EVENTHUB_CONNECTION_ACTIVITY_LOGS"
        // can validate with query in root terraform dir like `terraform output eventhub_connection_string`
        value = azurerm_eventhub_authorization_rule.eh_insights_auth_rule.primary_connection_string
      }
      env {
        name  = "SCC_ENDPOINT"
