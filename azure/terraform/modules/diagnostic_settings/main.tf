...
resource "azurerm_monitor_diagnostic_setting" "diag_setting" {
  # count                          = length(data.azurerm_monitor_diagnostic_categories.diag_cats.log_category_types) > 0 ? 1 : 0
  name                           = var.name
  ...
  dynamic "enabled_log" {
    ...
    for_each = data.azurerm_monitor_diagnostic_categories.diag_cats.log_category_types
    content {
