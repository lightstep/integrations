...
output "dashboard_url" {
  ...
  description = "OpenTelemetry Collector Connectedcache_ispcustomers Dashboard URL"
}

resource "lightstep_dashboard" "otel_collector_dashboard" {
  project_name   = var.lightstep_project
  dashboard_name = "Connectedcache_ispcustomers Metrics"
  dashboard_description = "Monitor Connectedcache_ispcustomers with this metrics overview dashboard."
...
EOT
  ...
  chart {
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_inboundbps_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location)   &&  (metadata_cachenodeid == $metadata_cachenodeid) )  | delta | group_by [], sum
EOT
    }
  }
...
EOT
  ...
  chart {
    name = "azure_outboundbps_average"
    rank = "7"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_outboundbps_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location)   &&  (metadata_cachenodeid == $metadata_cachenodeid) )  | delta | group_by [], sum
EOT
    }
  }
