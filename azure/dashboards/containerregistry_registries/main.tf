...
resource "lightstep_dashboard" "otel_collector_dashboard" {
  ...
  chart {
    name = "azure_agentpoolcputime_total"
    rank = "0"
    type = "timeseries"
...
EOT
  ...
  chart {
    name = "azure_successfulpullcount_total"
    rank = "3"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_successfulpullcount_total | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location) )  | delta | group_by [], sum
EOT
    }
  }

  chart {
    name = "azure_successfulpushcount_total"
    rank = "4"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_successfulpushcount_total | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location) )  | delta | group_by [], sum
EOT
    }
  }

  chart {
    name = "azure_totalpullcount_total"
    rank = "5"
    type = "timeseries"
...
EOT
  ...
  chart {
    name = "azure_totalpushcount_total"
    rank = "6"
    type = "timeseries"
