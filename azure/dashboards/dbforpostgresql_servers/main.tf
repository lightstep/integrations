...
resource "lightstep_dashboard" "otel_collector_dashboard" {
  ...
  dashboard_name = "Dbforpostgresql_servers Metrics"
...
EOT
  ...
  chart {
    name = "azure_connections_failed_total"
    rank = "2"
    type = "timeseries"
...
EOT
  ...
  chart {
    name = "azure_pg_replica_log_delay_in_bytes_maximum"
    rank = "8"
    type = "timeseries"
...
EOT
  ...
  chart {
    name = "azure_pg_replica_log_delay_in_seconds_maximum"
    rank = "9"
    type = "timeseries"
...
EOT
  ...
  chart {
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_serverlog_storage_usage_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location) )  | delta | group_by [], sum
EOT
    }
  }
