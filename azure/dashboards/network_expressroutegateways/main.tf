...
output "dashboard_url" {
  ...
  description = "OpenTelemetry Collector Network_expressroutegateways Dashboard URL"
}

resource "lightstep_dashboard" "otel_collector_dashboard" {
  project_name   = var.lightstep_project
  dashboard_name = "Network_expressroutegateways Metrics"
  dashboard_description = "Monitor Network_expressroutegateways with this metrics overview dashboard."

  chart {
    name = "azure_ergatewayconnectionbitsinpersecond_average"
    rank = "0"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_ergatewayconnectionbitsinpersecond_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location)   &&  (metadata_connectionname == $metadata_connectionname) )  | delta | group_by [], sum
EOT
    }
  }

  chart {
    name = "azure_ergatewayconnectionbitsoutpersecond_average"
    rank = "1"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_ergatewayconnectionbitsoutpersecond_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location)   &&  (metadata_connectionname == $metadata_connectionname) )  | delta | group_by [], sum
EOT
    }
  }
...
EOT
  ...
  chart {
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_expressroutegatewaynumberofvminvnet_maximum | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location) )  | delta | group_by [], sum
EOT
    }
  }
