...
output "dashboard_url" {
  ...
  description = "OpenTelemetry Collector Classicstorage_storageaccounts_fileservices Dashboard URL"
}

resource "lightstep_dashboard" "otel_collector_dashboard" {
  project_name   = var.lightstep_project
  dashboard_name = "Classicstorage_storageaccounts_fileservices Metrics"
  dashboard_description = "Monitor Classicstorage_storageaccounts_fileservices with this metrics overview dashboard."
...
EOT
  ...
  chart {
    name = "azure_filesharequota_average"
    rank = "5"
    type = "timeseries"
    ...
    query {
      ...
      hidden       = false
      query_string = <<EOT
metric azure_filesharequota_average | filter (  (azuremonitor.subscription_id == $azuremonitor_subscription_id)   &&  (azuremonitor.tenant_id == $azuremonitor_tenant_id)   &&  (azuremonitor.resource_id == $azuremonitor_resource_id)   &&  (location == $location)   &&  (metadata_fileshare == $metadata_fileshare) )  | delta | group_by [], sum
EOT
    }
  }
